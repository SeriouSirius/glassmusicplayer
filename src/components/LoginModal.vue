...
async function handlePhoneLogin() {
  ...
  phoneError.value = ''
  // 直接用 fetch 傳 password 明文（HTTPS 環境安全）
  ...
  <div class="modal-backdrop" @click.self="emit('close')">
    <div class="modal">
      ...
      <div v-if="activeTab === 'qr'" class="qr-panel">
        <div class="qr-wrap">
          <img v-if="qrImage && qrStatus !== 'expired'" :src="qrImage" class="qr-img" alt="登录二维码" />
...
.qr-overlay.expired { background: rgba(0,0,0,.75); }
.refresh-btn {
  background: var(--color-primary); color: #fff;
  border: none; border-radius: 6px;
  padding: 6px 14px; cursor: pointer;
}
.qr-hint { font-size: .875rem; color: var(--color-text-muted); }
.phone-panel { display: flex; flex-direction: column; gap: 14px; }
...
.form-error { font-size: .875rem; color: var(--color-error); }
.login-btn {
